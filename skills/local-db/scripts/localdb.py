...
_DESTRUCTIVE_PATTERNS = re.compile(
    ...
    r'DELETE\s+FROM|TRUNCATE|ALTER\s+TABLE\s+\S+\s+DROP)\b',
    re.IGNORECASE
)
...
def _split_sql(sql):
    ...
    while i < length:
        ...
        # Only increment for mid-statement BEGIN (trigger body), not for
        # transaction control (BEGIN / BEGIN TRANSACTION) which starts a statement.
        if ch in ('B', 'b') and sql[i:i + 5].upper() == 'BEGIN':
            ...
            if prev_ok and next_ok:
                ...
                if current_text:
                    begin_depth += 1
        elif ch in ('E', 'e') and sql[i:i + 3].upper() == 'END' and begin_depth > 0:
            prev_ok = i == 0 or not _word_char(sql[i - 1])
...
def cmd_execute(args):
    ...
    try:
        ...
        # Always commit before reading results: covers INSERT...RETURNING and
        # other write statements where SQLite reports rowcount == 0.
        conn.commit()
        ...
        if last_cursor and last_cursor.description:
            cols = [d[0] for d in last_cursor.description]
