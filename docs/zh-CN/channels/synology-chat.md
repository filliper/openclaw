...
2. 在 Synology Chat 集成设置中：
   ...
   - 创建一个 outgoing webhook 并设置你的 secret token。
...
- 在 `allowlist` 模式下，空的 `allowedUserIds` 列表被视为配置错误，webhook 路由将不会启动（如需允许所有用户，请使用 `dmPolicy: "open"`）。
...
每个账户可以覆盖 token、incoming URL、webhook path、DM 策略和限制。
...
- 入站 webhook 请求经过 token 验证并按发送者进行速率限制。
